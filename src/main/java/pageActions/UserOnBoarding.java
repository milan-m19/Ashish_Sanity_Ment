...
public static String SelectUserRole(String role) throws Exception {
	...
	try {
		...
		DikshaUtils.waitForElementToBeVisible(Ub.getWelcomeToDiksha());
		...
		else if (role.equals("Head Teacher & Official")) {
			DikshaUtils.waitToBeClickableAndClick(Ub.getHeadteacher());
			DikshaUtils.waitToBeClickableAndClick(Ub.getContinueButton());
		}
		Thread.sleep(3000);
		DikshaUtils.waitForElementToBeVisible(bmcpopup.getBoard());
	...
	public static void VerifyTheUserIsAbleToUpdateTheBMCAndLocationDetails() throws Exception {
		...
		LocationPopup locationpopup = PageFactory.initElements(driver, LocationPopup.class);
		Thread.sleep(1000);
		String RequiredDetailsText = "NA";
		String expected = "User should be able To Edit  Details Successfully";
		String actual = "User is unable To Edit Submit Details ";
		...
		DikshaUtils.waitToBeClickableAndClick(bmcpopup.getBoardC());
	//	DikshaUtils.waitToBeClickableAndClick(bmcpopup.getCISCE());
		Thread.sleep(2000);
		DikshaUtils.waitToBeClickableAndClick(bmcpopup.getBoard());
		Thread.sleep(2000);
		DikshaUtils.waitToBeClickableAndClick(bmcpopup.getCISCE());
...
//		DikshaUtils.waitToBeClickableAndClick(bmcpopup.getMedium());
//		Thread.sleep(2000);
//		DikshaUtils.waitToBeClickableAndClick(bmcpopup.getselectMediumValue());
		Actions act = new Actions(driver);
		act.moveByOffset(50, 100).click().build().perform();
		Thread.sleep(1000);
		...
		Thread.sleep(5000);
		String actualBoardName=home.getTextCISCEBoard().getText();
		String actualMediumName=home.getTextEnglishMedium().getText();
		String actualClassName=home.getTextClasss2().getText();
	        Assert.assertEquals(actualBoardName, "CISCE");
	        Assert.assertEquals(actualMediumName, "English");
	        Assert.assertEquals(actualClassName, "Class 2"); 
	        RequiredDetailsText = "text";
			Thread.sleep(2000);
			actual = "User is able to Edit Details Successfully";

		} finally {
			String ValidateText = RequiredDetailsText != null ? RequiredDetailsText : "N/A";
			Listeners.customAssert("Text", ValidateText, expected, actual);
		}
...
//		DikshaUtils.waitWebDriver(3000);
	...
	public static void validateDuringAddUserAndSwitchMUAuserUnableToGoFurtherWithoutSelectingCheckBox() throws Exception {
		String expect = "MUA should not be able to proceed further without accepting T&C popup";
		String actual = "MUA able to proceed further without accepting T&C popup";
		String addUserbtn = null;
		try {
			ManageUser adduser = PageFactory.initElements(driver, ManageUser.class);
			LoginPage loginpage = PageFactory.initElements(driver, LoginPage.class);
			
			DikshaUtils.waitToBeVisibleAndClick(loginpage.getHeaderDropdown());
			DikshaUtils.waitForElementToBeVisible(adduser.getaddAnotherUser());
			DikshaUtils.waitToBeClickableAndClick(adduser.getaddAnotherUser());
			DikshaUtils.waitForElementToBeVisible(adduser.getEnterName());
			DikshaUtils.waitToBeClickableAndClick(adduser.getEnterName());
			String name = DikshaUtils.set_Content_Name("Abc_");
			adduser.getEnterName().sendKeys(name);
			DikshaUtils.waitForElementToBeVisible(adduser.getAddUserTab());
			Assert.assertTrue(adduser.getAddUserTab().isDisplayed());
			DikshaUtils.waitForElementToBeVisible(adduser.getvalidateCancelButton());
			Assert.assertTrue(adduser.getvalidateCancelButton().isDisplayed());
			DikshaUtils.waitWebDriver(2000);
			DikshaUtils.waitForElementToBeVisible(adduser.getvalidateLabel());
			Assert.assertTrue(adduser.getvalidateLabel().isDisplayed());
			DikshaUtils.waitForElementToBeVisible(adduser.getAddUserTab());
			DikshaUtils.waitToBeClickableAndClick(adduser.getAddUserTab());
			DikshaUtils.waitForElementToBeVisible(adduser.getselectAddedUser());
			DikshaUtils.waitToBeClickableAndClick(adduser.getselectAddedUser());
			DikshaUtils.waitForElementToBeVisible(adduser.getchangeUser());
			DikshaUtils.waitToBeClickableAndClick(adduser.getchangeUser());
			DikshaUtils.waitForElementToBeVisible(adduser.getvalidateContinue());
			Assert.assertFalse(adduser.getvalidateContinue().isEnabled());
			DikshaUtils.waitForElementToBeVisible(adduser.getclickCheckBox());
			DikshaUtils.waitToBeClickableAndClick(adduser.getclickCheckBox());
			DikshaUtils.waitForElementToBeVisible(adduser.getcontinueButton());
			DikshaUtils.waitForElementToBeVisible(adduser.getcontinueButton());
			DikshaUtils.waitToBeClickableAndClick(adduser.getcontinueButton());
			actual = "MUA not able to proceed further successfully without accepting T&C popup";
			addUserbtn = "True";
		} finally {
			String Text = addUserbtn != null ? addUserbtn : "N/A";
			Listeners.customAssert("True", Text, expect, actual);
		}
	}
